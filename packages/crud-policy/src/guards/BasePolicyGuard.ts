...
export class BasePolicyGuard implements CanActivate {
  constructor(
    ...
    public opts: Pick<PolicyGuardOpts, 'extractors' | 'userPolicyField'>,
  ) {}
  ...
  public getRequiredPolicies(context: ExecutionContext): Policy[] | undefined {
    return this.reflector.getAllAndOverride<Policy[] | undefined>(POLICY_NAME_METADATA, [
      context.getHandler(),
      context.getClass(),
    ]);
  }

  public getAndValidateResourceId(
  ...
  public hasCorrectPolicies(context: ExecutionContext) {
    ...
    return validatePolicies(
      requiredPolicies,
      user[this.opts.userPolicyField] ?? [],
      entityId,
    );
  }
}
